...
public class ResourceOccupyEDF extends Thread {

	protected EDFJobInProgressListener jobListener;
	...
	ResourceOccupyEDF(EDFJobInProgressListener listen) {
		...
		for (int i = 0; i < 1850; i++) {
			...
			synchronized (jobQueue) {
				for (JobInProgress job : jobQueue) {
					try {
						if (job.getJobConf().getJobName().contains("flow1")) {
							flow1[i][0] += job.runningMaps();
							flow1[i][1] += job.runningReduces();
						}
						if (job.getJobConf().getJobName().contains("flow2")) {

							flow2[i][0] += job.runningMaps();
							flow2[i][1] += job.runningReduces();
						}
						if (job.getJobConf().getJobName().contains("flow3")) {
							flow3[i][0] += job.runningMaps();
							flow3[i][1] += job.runningReduces();
						}
						if (job.getJobConf().getJobName().contains("flow4")) {
