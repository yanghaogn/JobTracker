...
public class LSSTaskScheduler extends TaskScheduler {

  private static final int MIN_CLUSTER_SIZE_FOR_PADDING = 3;
  public static final Log LOG = LogFactory
      .getLog(LSSTaskScheduler.class);


  protected LSSJobInProgressListener lssJobInProgressListener;
  protected EagerTaskInitializationListener eagerTaskInitializationListener;
  private float padFraction;

  public LSSTaskScheduler() {
    this.lssJobInProgressListener = new LSSJobInProgressListener();
  ...
  public synchronized void start() throws IOException {
    ...
    lssJobInProgressListener.NUMMapSlots = this.taskTrackerManager.getClusterStatus().getMaxMapTasks();
  ...
  public synchronized void terminate() throws IOException {
    if (lssJobInProgressListener != null) {
  ...
  public synchronized List<Task> assignTasks(TaskTracker taskTracker)
    ...
    Collection<JobInProgress> jobQueue = lssJobInProgressListener.getJobQueue();
    ...
    synchronized (jobQueue) {
      ...
      lssJobInProgressListener.killSpillDeadline();
    ...
    for (int i = 0; i < availableMapSlots; ++i) {
      synchronized (jobQueue) {
        ...
        for (JobInProgress job : jobQueue) {
          ...
          if (0 == job.finishedMapTasks + job.runningMapTasks) {
            Task t = null;
            // Try to schedule a node-local or rack-local Map task
            t = job.obtainNewNodeOrRackLocalMapTask(
            ...
            if (t != null) {
              ...
              // task-failures,
              // speculative tasks etc. beyond the highest
              // priority job
              if (exceededMapPadding) {
                break scheduleMaps;
              }
              // Try all jobs again for the next Map task
              newJob = true;// newJob为true
              break;
            } else {
              // Try to schedule a node-local or rack-local Map
              // task
              ...
              if (t != null) {
                assignedTasks.add(t);
                ++numNonLocalMaps;
                // We assign at most 1 off-switch or speculative
                // task
  ...
  private boolean exceededPadding(boolean isMapTask,
    ...
    Collection<JobInProgress> jobQueue =
        lssJobInProgressListener.getJobQueue();
  ...
  public synchronized Collection<JobInProgress> getJobs(String queueName) {
    return lssJobInProgressListener.getJobQueue();
  }
