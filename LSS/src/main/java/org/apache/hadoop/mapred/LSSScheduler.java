...
import org.apache.hadoop.mapred.LSSJobInProgressListener.JobSchedulingInfo;
...
public class LSSScheduler extends TaskScheduler {
  ...
  protected LSSJobInProgressListener jobListener;
  protected EagerTaskInitializationListener eagerTaskInitializationListener;
  private float padFraction;

  public LSSScheduler() {
    this.jobListener = new LSSJobInProgressListener();
  ...
  public synchronized void start() throws IOException {
    ...
    new ResourceOccupyLSS(jobListener).start();
  ...
  public synchronized List<Task> assignTasks(TaskTracker taskTracker)
    ...
    Collection<JobInProgress> jobQueue = jobListener.getJobQueue();
    ...
    final int trackerCurrentMapCapacity = Math.min(11, Math.min(
    ...
    for (int i = 0; i < availableMapSlots; ++i) {
      synchronized (jobQueue) {
        /**
         * Add by user 对于未运行task的job，分配一个map task
         */
    ...
    //
    final int trackerCurrentReduceCapacity = Math.min(11, Math.min(
