...
public class LLFJobInProgressListener extends JobInProgressListener {
	/*
	 * ＊ 集群的状态
	 */
	public static int NUMMapSlots = 10;// 集群的Map Slots数目
	public static int NUMReduceSlots = 5;// 集群的Reduce Slots数目
	public static final Log LOGReOrder = LogFactory.getLog("LOGReOrder");
	...
	public LLFJobInProgressListener() {
	 ...
	 */
	protected LLFJobInProgressListener(
	...
	public synchronized void reOrderJobs() {
		synchronized (jobQueue) {
			...
			while (itrKey.hasNext()) {
				...
				JobInProgress job = info.jip;
				if (null == info) {
					writeFile("/NULL/" + System.currentTimeMillis(),
							"NULL=info");
				}
				if (null == job) {
					writeFile("/NULL/" + System.currentTimeMillis(), "NULL=job");
				}
				try {
					double AvgTaskTime = getAvgTaskTime(job);
					// 如果平均时间存在
					if (0 < AvgTaskTime) {
						// 任务的平均时间
						/*
						 * unFinishedd map task数目
						  
						 */
						int NumMaps = job.desiredMaps() - job.finishedMaps();
						int NumReduce = job.desiredReduces()
								- job.finishedReduces();// unFinishedd reduce
														// task数目
						int numMapSlot = NumMaps
								/ LLFJobInProgressListener.NUMMapSlots;
						int numReduceSlot = NumReduce
								/ LLFJobInProgressListener.NUMReduceSlots;

						if (numMapSlot * LLFJobInProgressListener.NUMMapSlots < NumMaps)
							numMapSlot++;

						if (numReduceSlot
								* LLFJobInProgressListener.NUMReduceSlots < NumReduce)
							numReduceSlot++;
						...
						if (0 < job.runningMaps()) {
							TaskInProgress[] maps = job.maps;
						...
						// remaingTime-=正在运行的reduce task的执行时间／reduce槽数
						if (0 < job.runningReduces()) {
							// 加上正在运行的reduce task的执行时间
							TaskInProgress[] reduces = job.reduces;
					...
					String content = "Exception:"
							...
							+ e.getMessage()
							+ "\n"
							+ (System.currentTimeMillis() - info.jip
									.getStartTime());
					writeFile("/Exception1/" + System.currentTimeMillis(),
							content);
				}
			...
			Map<JobSchedulingInfo, JobInProgress> newQueue = new TreeMap<JobSchedulingInfo, JobInProgress>(
					LLF_JOB_QUEUE_COMPARATOR);
			itrKey = jobQueue.keySet().iterator();
			while (itrKey.hasNext()) {
				JobSchedulingInfo info = (JobSchedulingInfo) itrKey.next();
	...
	public synchronized void killSpillDeadline() {
		synchronized (jobQueue) {
			//ArrayList<JobSchedulingInfo> killedJobs = new ArrayList<JobSchedulingInfo>();
			Iterator iterator = jobQueue.keySet().iterator();
			while (iterator.hasNext()) {
				JobSchedulingInfo info = (JobSchedulingInfo) iterator.next();
